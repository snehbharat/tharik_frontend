...
export const TaskManagement = ({ isArabic, currentUser }) => {
  ...
  const loadTasks = async () => {
    try {
      ...
      const filters = {
        tab: activeTab,
        ...(currentUser?.role === 'employee' && { assigned_to: currentUser.id })
      };
  ...
  const handlePhotoUpload = async (taskId, files) => {
    ...
    try {
      ...
      if (uploadedPhotos.length > 0) {
        const response = await ApiService.addPhotoUpdate(taskId, uploadedPhotos);
  ...
  const handleSendSMS = async (taskId) => {
    ...
    try {
      ...
      const smsUpdateData = {
        ...
        textAr: `تم إرسال تذكير SMS: "${smsMessage.messageAr || smsMessage.message}"`,
        type: "sms"
      };
  ...
  const handleTaskUpdate = async (taskId, updates) => {
    try {
      ...
      if (response.response) {
        // Show success message
        alert(isArabic ? "تم تحديث المهمة بنجاح!" : "Task updated successfully!");
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12 text-gray-500">
              <p className="text-lg">
                {isArabic ? "لا توجد مهام في هذا القسم" : "No tasks found in this section"}
              </p>
              <button
                onClick={() => setShowNewTask(true)}
