...
const filterEmployees = (employees, searchTerm, filters) => {
  return employees.filter(emp => {
    const matchesSearch = !searchTerm || 
      ...
      emp.professionalInfo?.jobTitle?.toLowerCase().includes(searchTerm.toLowerCase());
    
    const matchesNationality = !filters.nationality || emp.nationality === filters.nationality;
    ...
    return matchesSearch && matchesNationality && matchesPosition && matchesStatus;
  });
};
...
const EmployeeManagementContent = ({ isArabic }) => {
  ...
  const uniqueNationalities = useMemo(() => 
    [...new Set(employeesData.map(emp => emp.nationality))].filter(Boolean).sort(), [employeesData]
  );
  const uniquePositions = useMemo(() => 
    [...new Set(employeesData.map(emp => emp.position))].filter(Boolean).sort(), [employeesData]
  );
  ...
  const expiringDocs = employeesData.filter(emp => 
    emp.documents && emp.documents.some(doc => getDaysUntilExpiry(doc.expiryDate) <= 30)
  ).length;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 flex-wrap">
        ...
        <select 
          ...
          className="border border-gray-300 rounded-lg px-3 py-2"
        >
          <option value="">{isArabic ? 'جميع المناصب' : 'All Positions'}</option>
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                filteredEmployees.map((employee) => {
                  ...
                  const departmentName = getDepartmentName(employee.professionalInfo?.departmentId?._id);
                  const expiringDocs = employee.documents ? employee.documents.filter(doc => 
                  ...
                  return (
                    <tr key={employee._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-4">
                        ...
                        <div className="text-sm text-gray-500">{employee.professionalInfo?.jobTitle}</div>
                      </td>
                      <td className="px-4 py-4">
                        <div className="text-sm text-gray-900">
                          {departmentName || employee.professionalInfo?.departmentId?.name}
                        </div>
                      </td>
                      <td className="px-4 py-4">
                        <div className="text-sm text-gray-900">
                      ...
                      <td className="px-4 py-4">
                        <div className="text-sm text-gray-900">
                          {employee.hire_date ? new Date(employee.hire_date).toLocaleDateString() : 
                           employee.professionalInfo?.hireDate ? new Date(employee.professionalInfo.hireDate).toLocaleDateString() : 'N/A'}
                        </div>
                      </td>
                      <td className="px-4 py-4">
