...
const StatCard = ({
  ...
  <div
    className={`${bgColor} rounded-xl p-6 border ${bgColor.replace(
      "bg-",
      "border-"
    )}`}
  >
    <div className="flex items-center gap-3 mb-3">
      ...
      <div>
        ...
        <div className={`text-sm ${textColor.replace("900", "700")}`}>
          {title}
        </div>
      </div>
    </div>
...
const EmployeeStatistics = ({ employees, isArabic, loading }) => {
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className="bg-gray-200 animate-pulse rounded-xl p-6 h-32"
          ></div>
        ))}
      </div>
    );
  ...
  const statisticsData = [
    ...
    },
    {
      ...
      value: stats.active,
      subtitle: `${stats.retentionRate.toFixed(1)}% ${
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">
              {stats.total > 0
                ? ((stats.newHires / stats.total) * 100).toFixed(1)
                : 0}
              %
            </div>
            <div className="text-sm text-gray-600">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-yellow-600">
              {stats.total > 0
                ? ((stats.onLeave / stats.total) * 100).toFixed(1)
                : 0}
              %
            </div>
            <div className="text-sm text-gray-600">
