...
class InvoiceService {
  ...
  createInvoice(payload) {
    return apiClient.post("/payable-invoice/create", payload);
  }

  // Get all invoices with pagination
  getAllInvoices(page = 1, limit = 10) {
    return apiClient.get("/payable-invoice/getAll", { page, limit });
  }

  // Get invoice by ID
  getInvoiceById(id) {
    return apiClient.get(`/payable-invoice/${id}`);
  }

  // Update invoice
  updateInvoice(id, payload) {
    return apiClient.put(`/payable-invoice/${id}`, payload);
  }

  // Delete invoice
  deleteInvoice(id) {
    return apiClient.delete(`/payable-invoice/${id}`);
  }
